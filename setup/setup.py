...
def insert():
    ...
    directors = pd.DataFrame(directors_dict.items())[[1, 0]].rename(columns={1: 'director_id', 0: 'director_name'})
    directors.to_sql('directors', engine, if_exists='append', index=False)

    print("Populating tags...")
    tags = dfs[FILES[4]].rename(columns={'id': 'tag_id', 'value': 'tag_name'})
    tags.to_sql('tags', engine, if_exists='append', index=False)

    print("Populating movies...")
    movies = dfs[FILES[3]][['id', 'title', 'year', 'imdbPictureURL']].rename(columns={'id': 'movie_id', 'imdbPictureURL': 'imdb_picture_url'})
    ...
    directs = dfs[FILES[1]][['director_id', 'movieID']].rename(columns={'movieID': 'movie_id'})
    directs.to_sql('directs', engine, if_exists='append', index=False)

    print("Populating user_tags...")
    user_tags = dfs[FILES[0]].drop(columns=['timestamp']).rename(columns={'userID': 'user_id', 'movieID': 'movie_id', 'tagID': 'tag_id'})
    user_tags.to_sql('user_tags', engine, if_exists='append', index=False)

    print("Populating ratings...")
    ratings = dfs[FILES[5]][['userID', 'movieID', 'rating']].rename(columns={'userID': 'user_id', 'movieID': 'movie_id'})
    ...
    genre = pd.DataFrame(genre_dict.items())[[1, 0]].rename(columns={1: 'genre_id', 0: 'genre_name'})
    ...
    has_genre = dfs[FILES[2]][['genre_id', 'movieID']].rename(columns={'movieID': 'movie_id'})
