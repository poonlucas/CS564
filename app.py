...
def create_app():
    ...
    def get_movie_recommendation():
        ...
        sql = f'''SELECT top_movies.title, top_movies.average_rating, g.genre_name, t.tag_name
        FROM (
        ...
        for title, (average_rating, genre, tags) in movie_dict.items():
            movies.append((title, average_rating, ', '.join(str(genre) for genre in genre), ', '.join(str(tag) for tag in tags)))
